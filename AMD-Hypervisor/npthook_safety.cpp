	...
	void PageSynchronizationPatch()
	{
		...
		for (int i = 0; i < pe_hdr->FileHeader.NumberOfSections; ++i)
		{
			...
			if (!strcmp((char*)section[i].Name, "PAGE"))
			{
				...
				uintptr_t clean_process_address_space = RELATIVE_ADDR(Utils::FindPattern((uintptr_t)start, section[i].Misc.VirtualSize, "\xE8\xCC\xCC\xCC\xCC\x8B\x5D\x00", 8, 0xCC), 1, 5);
